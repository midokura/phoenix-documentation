...
- **SSH private keys** are stored locally on the operator workstation (`~/.ssh/`). The corresponding public key is registered in OpenStack (`nova keypair`) and in `authorized_keys` on all hosts.
