...
**`ceph_rgw_tls_enabled`** · default: `false`

Enables TLS on the RGW service spec. When `true`, the provisioning playbook reads a pre-generated certificate and vault-encrypted key from `assets/ceph/` and applies a TLS-enabled cephadm service spec. The certificate must exist before running `--provision-ceph` — see [Generate RGW TLS certificate](#generate-rgw-tls-certificate) below.

```yaml
ceph_rgw_tls_enabled: true
```

---

**`ceph_rgw_tls_cn`** · *required when TLS enabled*

Common Name used in the self-signed certificate. This is also the hostname DNS resolves to for the RGW endpoint (configured via OpenWRT dnsmasq host overrides).

```yaml
ceph_rgw_tls_cn: storage.isys
```

---

**`ceph_rgw_hosts`** · *required when TLS enabled*

List of host, IP, and port entries that control RGW daemon placement. The hostnames and IPs listed here must be covered by the SANs in the TLS certificate generated by `generate-rgw-tls.<env>.sh` — the script does not read this variable, so they must be kept in sync manually.
...
### Generate RGW TLS certificate

:::note

Skip this section if `ceph_rgw_tls_enabled` is `false` in your inventory.

:::

When `ceph_rgw_tls_enabled: true`, the provisioning playbook expects a pre-generated certificate and vault-encrypted private key under `assets/ceph/` before it runs. The playbook does not generate these files — if they are absent, the first task fails immediately.

From the gpu-infrastructure repository root, run the environment-specific generation script, passing the path to the inventory's assets directory:
...
The script will prompt for the Ansible Vault password, then produce:

| File | Description |
|------|-------------|
| `assets/ceph/rgw-tls.crt` | Public certificate — commit as-is |
| `assets/ceph/rgw-tls.key` | Vault-encrypted private key — commit |
| `assets/config/certificates/ca/ceph-rgw.crt` | CA copy for Kolla container trust — commit |

Commit all three files to phoenix-inventories:
...
git commit -m "chore(<env>): generate RGW TLS certificate"
git push
```

Then upload the generated files to the bastion before running `--provision-ceph`. The bastion expects them under `release-assets/assets/`, which is the directory mounted as `/infra-management` inside the Ansible container:
...
The certificate is valid for 730 days. For renewal procedures, see the [Rotate Ceph RGW TLS Certificate](./runbooks/key-rotation/CEPH_RGW_TLS) runbook.
...
| `ceph_rgw_tls` | Apply TLS-enabled RGW service spec and redeploy RGW daemons (use for cert renewal) |
